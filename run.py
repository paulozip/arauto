...
## Dropdown to select file source
file_options = ["Use data sent from REST API", "Upload a file"]
selected_upload_option = st.sidebar.selectbox("File Source", file_options, 0)
df = None
if selected_upload_option == "Use data sent from REST API":
    filename, df = file_selector()
else:
    file_formats = ["CSV or TXT", "Excel"]
    selected_file_format = st.sidebar.selectbox("File format", file_formats, 0)
    filename = selected_file_format
    df = file_uploader(selected_file_format)

# Avoid use before assignment error
if df is not None:
    st.markdown("## **First lines of your data**")
    st.dataframe(df.head(10))  # First lines of DataFrame

    ds_column, y, data_frequency, test_set_size, exog_variables = sidebar_menus(
    ...
    force_transformation = sidebar_menus(
        "force_transformations"
    )  # You can force a transformation technique
    ...
    if ("Custom Difference") in force_transformation:
        ...
        seasonal_difference_size = st.sidebar.slider(
            "Seasonal Difference size: ", 0, 30, 1
        )
    ...
    if force_transformation == None:
        ts, d, D, seasonality, acf_pacf_data, transformation_function, test_stationarity_code = test_stationary(
            df[y], plot_adfuller_result, data_frequency
        )
    else:
        ts, d, D, seasonality, acf_pacf_data, transformation_function, test_stationarity_code = test_stationary(
            df[y],
            plot_adfuller_result,
            data_frequency,
    ...
    st.markdown(
        "**Suggested parameters for your model**: {}x{}{}".format(
            (p, d, q), (P, D, Q), (seasonality)
        )
    )
    ...
    try:
        p, d, q, P, D, Q, s, train_model, periods_to_forecast, execute_grid_search = sidebar_menus(
            "terms", test_set_size, seasonality, (p, d, q, P, D, Q, seasonality), df=ts
        )
    except ValueError:
        error_message = """
    ...
    if train_model:
        ...
        try:
            model = train_ts_model(
                train_set, p, d, q, P, D, Q, s, exog_variables=exog_train, quiet=False
            )
        except ValueError as ve:
            if ve.args[0] == "maxlag should be < nobs":
                raise ValueError(
                    "Seems that you don't have enough data. Try to use smaller terms for AR and MA (p, q, P, Q)"
                )
            else:
                raise ve
        ...
        st.write(
            "The model was trained with this data. It's trying to predict the same data"
        )
        if transformation_function == np.log1p:
            predict_set(
                ...
                np.expm1,
                model,
                show_train_prediction=show_train_prediction,
                show_test_prediction=show_test_prediction,
            )
        else:
            predict_set(
                train_set.iloc[-24:],
        ...
        st.write(
            "Unseen data. The model was not trained with this data and it's trying to forecast"
        )
        if transformation_function == np.log1p:
            predict_set(
                test_set,
        ...
        if execute_grid_search:
            ...
            p, d, q, P, D, Q, s = grid_search_arima(
                train_set,
                exog_train,
                range(p + 2),
                range(q + 2),
        ...
        with st.spinner("Training model with entire dataset. Please wait."):
            final_model = train_ts_model(
                transformation_function(ts),
                p,
                d,
                q,
                P,
        ...
        if type(exog_variables) == type(pd.DataFrame()):
            st.write(
                "You are using exogenous variables. We can't forecast the future since we don't have the exogenous variables for future periods. Adapt the code below to use them."
            )
        else:
            if transformation_function == np.log1p:
                forecasts = np.expm1(final_model.forecast(periods_to_forecast))
                confidence_interval = np.expm1(
                    final_model.get_forecast(periods_to_forecast).conf_int()
                )

            else:
                forecasts = final_model.forecast(periods_to_forecast)
                confidence_interval = final_model.get_forecast(
                    periods_to_forecast
                ).conf_int()
